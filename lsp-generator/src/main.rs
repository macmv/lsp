...
pub fn main() {
  ...
  lsp.generate_notifications(
    &mut Generator::new("src/notification.rs", &names),
    &spec.notifications,
  );
...
impl LspGenerator<'_> {
  ...
  fn generate_struct_fields(
    ...
    for field in fields.iter() {
      ...
      if field.optional {
        if matches!(&field.ty, Type::Reference { name } if *name == struct_name) {
          g.write("Option<Box<");
          self.write_type(g, &field.ty, name_hints);
          g.write(">>");
          ...
          self.write_type(g, &Type::Or { items }, name_hints);
        } else {
          g.write("Option<");
          self.write_type(g, &field.ty, name_hints);
          g.write(">");
    ...
    for extends in extends {
      if let Some(name) = self.should_inline(&extends) {
        ...
        self.write_type(g, &extends, vec![]);
        g.writeln(",");
      }
    }
  }

  fn should_inline(&self, ty: &Type) -> Option<String> {
    match ty {
  ...
  fn generate_enum(&self, g: &mut Generator, ty: &Enumeration) {
    g.writeln("");
    g.write_doc(&ty.documentation);
    match ty.ty {
  ...
  fn generate_type_alias(&self, g: &mut Generator, ty: &TypeAlias) {
    g.writeln("");
    g.write_doc(&ty.documentation);
    match &ty.ty {
      Type::Or { items } => {
      ...
      _ => {
        g.writeln(format_args!("pub type {} = ", ty.name));
        self.write_type(g, &ty.ty, vec![ty.name.clone()]);
        g.writeln(";");
      }
    }
  }

  fn generate_requests(&self, g: &mut Generator, requests: &[Request]) {
    ...
    for n in requests {
      ...
      self.write_type(g, &n.result, vec![format!("{name}Result")]);
      g.writeln(format_args!(";"));
  ...
  fn generate_notifications(&self, g: &mut Generator, notifications: &[Notification]) {
  ...
  fn write_type(&self, g: &mut Generator, ty: &Type, name_hint: Vec<String>) {
    match ty {
      ...
      Type::Or { items } => {
        ...
        if items.len() == 1 {
          ...
          g.write("Or2<");
          self.write_type(g, &items[0], name_hint.clone());
          g.write(", ");
          self.write_type(g, &items[1], name_hint);
          g.write(">");
        } else if items.len() == 3 {
          g.write("Or3<");
          self.write_type(g, &items[0], name_hint.clone());
          g.write(", ");
          self.write_type(g, &items[1], name_hint.clone());
          g.write(", ");
          self.write_type(g, &items[2], name_hint);
          g.write(">");
        } else if items.len() == 4 {
          g.write("Or4<");
          self.write_type(g, &items[0], name_hint.clone());
          ...
          self.write_type(g, &items[1], name_hint.clone());
          g.write(", ");
          self.write_type(g, &items[2], name_hint.clone());
          g.write(", ");
          self.write_type(g, &items[3], name_hint);
          g.write(">");
        } else {
      ...
      Type::Array { element } => {
        g.write("Vec<");
        self.write_type(g, element, name_hint);
        g.write(">");
      }
      ...
      Type::Map { key, value } => {
        g.write("HashMap<");
        self.write_type(g, key, vec![]);
        g.write(", ");
        self.write_type(g, value, vec![]);
        g.write(">");
      }
